...
const buttonVariants = cva(
  'inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0',
  {
    variants: {
      ...
      gooeyColor: {
        default: 
          cn('from-white/40 dark:from-black/40',
            'before:from-white/40 dark:before:from-black/40',
            'after:from-white/40 dark:after:from-black/40'),
        destructive: 
          cn('from-black/40 dark:from-white/40',
            'before:from-black/40 dark:before:from-white/40',
            'after:from-black/40 dark:after:from-white/40'),
        outline: 
          cn('from-black/40 dark:from-white/40',
            'before:from-black/40 dark:before:from-white/40',
            'after:from-black/40 dark:after:from-white/40'),
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps & ButtonIconProps>(
  ({ className, variant, effect, size, gooeyColor, icon: Icon, iconPlacement, asChild = false, ...props }, ref) => {
    const Comp = asChild ? Slot : 'button';

    return (
      <Comp className={cn(buttonVariants({ variant, effect, size, gooeyColor, className }))} ref={ref} {...props}>
        {Icon &&
          iconPlacement === 'left' &&
          (effect === 'expandIcon' ? (
